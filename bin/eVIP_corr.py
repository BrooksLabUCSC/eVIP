...
def main():
    ...
    parser.add_argument("-zscore_gct",
                    help="""Z-score input gct file (spearman rank correlations
                                        will be calculated from this file)""")
...
def run_main(input=None, zscore_gct=None, out_dir=None):
    ...
    if input != None:
        ...
        data = pd.read_csv(file_path, delimiter="\t", skiprows=2, index_col=False,low_memory= False)
    ...
    # if the input is zscores
        ...
        with open(zscore_gct, "r") as zscores:
            ...
            # saving second line (#data rows, # data columns,
            # #row metadata fields, #col metadata fields)
            ...
            with open(out_dir + "/spearman_rank_matrix.gct", "w") as sp_output:
                ...
                data = pd.read_csv(zscore_gct, delimiter="\t",
                                        skiprows=2+num_meta_lines, index_col=False,low_memory= False)
                ...
                # writing matrix to output file
                matrixToFile_gct(sp_matrix[0], sp_output, sample_names,
                                    zscore_gct, num_meta_lines)
...
def matrixToFile_gct(matrix, output_file, header, zscore_gct, num_meta_lines):
    ...
    z_data = pd.read_csv(zscore_gct, delimiter="\t", skiprows=2, index_col=False,low_memory= False)
    column_lists = []
